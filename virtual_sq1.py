...
Classes:
    Square1: A virtual Square-1 object.
        ...
        _error_detected(input_type, errored_input, error_turns, error_turns_index):
        ...
        apply_alg(alg, for_case):
            ...
            If `for_case = True`: Changes the Square1's state so that the input algorithm `alg` brings it to its current state.
        apply_state(state):
...
class Square1:
    ...
    def _error_detected(self, input_type:int, errored_input, error_turns:list=[], error_turns_index:int=0) -> None:
        """
